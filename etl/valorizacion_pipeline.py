...
def _construir_hoja_proyecciones(
    ...
    """
    Construye la hoja Proyecciones con horizonte relativo a AÑO_FIN_DATOS
    (Fix 47): arranca AÑOS_LANZAMIENTO_PROGRAMA_NUEVO años después del último dato
    real (tiempo estimado de diseño curricular + registro calificado + apertura de
    la primera cohorte) y cubre HORIZONTE_PROYECCION_VALORIZACION años desde ahí.
    Con AÑO_FIN_DATOS=2024 esto reproduce el horizonte anterior (2027-2031).
    ...
    # Fix 47: antes año calendario fijo [2027..2031]; ahora relativo a AÑO_FIN_DATOS.
    AÑOS_LANZAMIENTO_PROGRAMA_NUEVO = 3  # años hasta la primera cohorte posible
    HORIZONTE_PROYECCION_VALORIZACION = 5  # años de proyección desde el lanzamiento
    _AÑO_INICIO_PROY = AÑO_FIN_DATOS + AÑOS_LANZAMIENTO_PROGRAMA_NUEVO
    AÑOS_PROY = list(range(_AÑO_INICIO_PROY, _AÑO_INICIO_PROY + HORIZONTE_PROYECCION_VALORIZACION))
    N_MIN = 2
    N_AÑOS_MIN = 3
    ...
    for _, prow in progs_unicos.iterrows():
        ...
        # Fix 47: primer año del horizonte, ya no literal "2027".
        proy_cat_inicio = _reg_lineal(serie_cat, AÑOS_PROY[0])
        valor_base = proy_cat_inicio * partic if proy_cat_inicio and proy_cat_inicio > 0 else 0.0
...
def run_fase_valorizacion(log: Callable = print) -> Path:
            ...
            filas.append(
                {
                    ...
                    "CATEGORIA": cat_raw,
                    "NIVEL": nivel,
                    # Fix 46: nivel que REALMENTE domina la categoría en el mercado y en
                    # los referentes (puede diferir de NIVEL, que es el nivel del
                    # programa candidato). Ver comentario de celda en el Excel.
                    "M_nivel_predominante_mercado": met_m_s.get("NIVEL_MAYORIT", ""),
                    "R_nivel_predominante_referentes": met_r_s.get("NIVEL_MAYORIT", ""),
    ...
    with pd.ExcelWriter(ruta, engine="openpyxl") as writer:
        ...
        # ── Hoja 3: Proyecciones (horizonte relativo a AÑO_FIN_DATOS, Fix 47) ──
...
def _reordenar_columnas_valorizacion(df_out: pd.DataFrame) -> pd.DataFrame:
    ...
    _cols_id = [
        ...
        "M_nivel_predominante_mercado", "R_nivel_predominante_referentes",  # Fix 46
...
def _formatear_hoja_valorizacion(writer, df_out: pd.DataFrame) -> None:
    ...
    N_ID = 9  # PROG_ID, CAT_ID, Categoría, Nivel, Nivel predom. mercado, Nivel predom.
              # referentes, Programa, ¿Tiene estudio?, Región (Fix 46: +2 columnas)
    ...
    NOMBRES = {
        ...
        "CAT_ID": "ID Categoría",
        "CATEGORIA": "Categoría",
        "NIVEL": "Nivel (programa candidato)",
        "M_nivel_predominante_mercado": "Nivel predom. mercado",
        "R_nivel_predominante_referentes": "Nivel predom. referentes",
    ...
    for ci, col in enumerate(cols, 1):
        ...
        elif col == "M_nivel_predominante_mercado":
            cell.comment = Comment(
                "Nivel de formación que realmente domina esta categoría en el mercado "
                "(no necesariamente el mismo que 'Nivel'). Todos los scores M_* se "
                "calculan con los umbrales del nivel del programa candidato, no de este "
                "nivel — por diseño: la pregunta es 'cómo le iría a EAFIT lanzando ESTE "
                "nivel', no 'cómo está el mercado en general'. Cuando difiere de 'Nivel', "
                "M_calificación puede no coincidir con la Calificación final del Estudio "
                "de Mercado de esta misma categoría — es esperado, no un error.",
                "SNIESManager",
            )
        elif col == "R_nivel_predominante_referentes":
            cell.comment = Comment(
                "Igual que 'Nivel predom. mercado', pero para los referentes "
                "(universidades competidoras nacionales).",
                "SNIESManager",
            )
    ...
    ANCHOS = {
        ...
        "CATEGORIA": 32,
        "NIVEL": 16,
        "M_nivel_predominante_mercado": 18,
        "R_nivel_predominante_referentes": 18,
        "PROGRAMA_EAFIT": 36,
