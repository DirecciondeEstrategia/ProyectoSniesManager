...
def _proyectar_primer_curso(
    ...
    Agrega las columnas:
      ...
      - AAGR_PROYECTADO           tasa de crecimiento anual proyectada, estilo CAGR,
                                  entre el último dato real (AÑO_FIN_DATOS) y el cierre
                                  del horizonte (año_fin_proyeccion):
                                  (PROYECCION_PC_{fin} / PC_{AÑO_FIN_DATOS}) ** (1/n_años) - 1
                                  Mismas unidades que AAGR_ROBUSTO (tasa anual, no
                                  promedio de estudiantes) — comparable directamente
                                  contra el crecimiento histórico de la misma categoría.
                                  Requiere PC_{AÑO_FIN_DATOS} > 0; si es 0, queda NaN.
      - R2_PROYECCION             R² del modelo ganador (el usado para proyectar)
      - TIPO_PROYECCION           Etiqueta de dirección del mercado proyectado,
                                  calculada sobre AAGR_PROYECTADO con umbrales
                                  propios calibrados por percentiles reales de esta
                                  métrica (Fix 45 — NO son los del AAGR histórico:
                                  su rango es mucho más angosto y los umbrales deben
                                  respetar el signo, ya que aquí la etiqueta indica
                                  dirección, no calidad):
                                  'EXPANSION'     si tasa > 5.7%   (P80)
                                  'CRECIENTE'     si 1.1% < tasa <= 5.7%   (P40-P80)
                                  'ESTABLE'       si 0% < tasa <= 1.1%    (P0-P40 positivo)
                                  'DECRECIENTE'   si -5.4% < tasa <= 0%   (P10-P20 negativo)
                                  'CONTRACCION'   si tasa <= -5.4%  (P10)
                                  'SIN_PROYECCION' si no hay suficientes datos

    Fix 43: reemplaza PENDIENTE_PROYECCION (promedio absoluto de estudiantes/año,
    correlación 0.76 con el tamaño de la categoría medida sobre datos reales) por
    esta tasa anualizada, comparable entre categorías de cualquier tamaño.
    Fix 45: corrige que DECRECIENTE/CONTRACCION podían asignarse a tasas positivas
    (127/127 categorías DECRECIENTE tenían tasa positiva antes de este fix) por
    reutilizar sin ajustar los cortes de score del AAGR histórico.
    """
    ...
    ag["AJUSTE_MODELO_FIN_DATOS"] = np.nan  # interno — Fix 44, auditoría del "salto"
    ...
    for idx in ag.index:
        ...
        # Fix 44: valor que el PROPIO modelo (misma coef_lin/coef_log ya ajustada)
        # predice en AÑO_FIN_DATOS. Evita comparar la proyección final contra el
        # dato real cuando la curva ajustada no pasa por el último punto real
        # (ver diagnóstico Fix 44 — "salto" que producía CONTRACCIÓN espuria en
        # 39/275 categorías con trayectoria proyectada ascendente).
        if usar_log:
            ajuste_fin_datos = np.polyval(coef_log, np.log(AÑO_FIN_DATOS - 2013))
        else:
            ajuste_fin_datos = np.polyval(coef_lin, AÑO_FIN_DATOS)
        ag.at[idx, "AJUSTE_MODELO_FIN_DATOS"] = ajuste_fin_datos

        # Base de comparación: el ajuste del modelo en AÑO_FIN_DATOS. Si el modelo
        # da un valor no positivo ahí (series cortas o muy ruidosas), se usa el
        # dato real como respaldo en vez de perder la proyección por completo.
        base_comparacion = ajuste_fin_datos if ajuste_fin_datos > 0 else pc_real_2024

        # Fix 43: tasa anualizada estilo CAGR, no promedio absoluto de estudiantes.
        if pd.notna(base_comparacion) and pd.notna(proy_fin) and float(base_comparacion) > 0:
            ag.at[idx, "AAGR_PROYECTADO"] = (
                (float(proy_fin) / float(base_comparacion)) ** (1 / n_años_horizonte) - 1
            )

    # Fix 35 / Fix 43 / Fix 45: TIPO_PROYECCION — etiqueta de dirección del mercado
    # proyectado. Fix 45 reemplaza los cortes del AAGR histórico (pensados para un
    # score de calidad, no para una etiqueta de dirección) por umbrales propios,
    # calibrados sobre los percentiles reales de AAGR_PROYECTADO (P10, P40, P80 de
    # la corrida de referencia), y separados explícitamente por signo para que
    # DECRECIENTE/CONTRACCION nunca se asignen a una tasa positiva.
    def _clasificar_tipo_proyeccion(tasa: float) -> str:
        if pd.isna(tasa):
            return "SIN_PROYECCION"
        if tasa > 0.057:      # P80 real ≈ +5.7%
            return "EXPANSION"
        if tasa > 0.011:      # P40 real ≈ +1.1%
            return "CRECIENTE"
        if tasa > 0.00:
            return "ESTABLE"
        if tasa > -0.054:     # P10 real ≈ -5.4%
            return "DECRECIENTE"
        return "CONTRACCION"

    ag["TIPO_PROYECCION"] = ag["AAGR_PROYECTADO"].apply(_clasificar_tipo_proyeccion)
...
_FIX26_PROYECCION_COLS = (
    ...
    + ["AAGR_PROYECTADO", "R2_PROYECCION", "TIPO_PROYECCION"]
...
def _escribir_hoja_estandar(writer: pd.ExcelWriter) -> None:
    ...
    SEÑALES = [
        ("▲ ACELERANDO", "C8E6C9", "1B5E20",
         f"Var. {_yr_ant}→{_yr_fin} ≥ +10% y no viene frenándose (diferencial vs. AAGR histórico ≥ −5 pp). "
         f"Crece fuerte y de forma consistente con su tendencia."),
        ("→ ESTABLE", "FFF9C4", "827717",
         f"Var. {_yr_ant}→{_yr_fin} ≥ 0%, sin ninguna de las alertas de las otras señales. "
         f"Crecimiento nulo o positivo sin sorpresas frente a su historia."),
        ("▼ DESACELERANDO", "FFE0B2", "E65100",
         f"Var. {_yr_ant}→{_yr_fin} ≥ 0% (sigue en positivo), pero muy por debajo de su propio AAGR "
         f"histórico (diferencial < −10 pp). Frena aunque técnicamente no cae."),
        ("↓ EN DECLIVE", "FFCDD2", "B71C1C",
         f"Var. {_yr_ant}→{_yr_fin} negativa y peor de lo que su historia hacía esperar "
         f"(diferencial < −10 pp frente al AAGR). La caída es una sorpresa negativa."),
        ("↓↓ CONTRACCION", "EF9A9A", "B71C1C",
         f"Var. {_yr_ant}→{_yr_fin} negativa pero dentro de lo esperable para esa categoría "
         f"(diferencial ≥ −10 pp). No es necesariamente menos severa que EN DECLIVE en magnitud — "
         f"puede ser una caída grande si el mercado ya venía en declive crónico."),
        ("— SIN DATO", "EEEEEE", "757575",
         f"No hay dato de primer_curso en {_yr_ant} o {_yr_fin} para calcular la variación."),
    ]

    SEÑALES_POST_PANDEMIA = [
        ("▲ ACELERANDO", "C8E6C9", "1B5E20",
         f"Var. {_yr_ant}→{_yr_fin} ≥ +10% y no viene frenándose (diferencial vs. AAGR post-pandemia "
         f"2022-{_yr_fin} ≥ −5 pp). Crece fuerte y de forma consistente con su tendencia reciente."),
        ("→ ESTABLE", "FFF9C4", "827717",
         f"Var. {_yr_ant}→{_yr_fin} ≥ 0%, sin ninguna de las alertas de las otras señales, comparado "
         f"contra la tendencia post-pandemia."),
        ("▼ DESACELERANDO", "FFE0B2", "E65100",
         f"Var. {_yr_ant}→{_yr_fin} ≥ 0% (sigue en positivo), pero muy por debajo de su AAGR "
         f"post-pandemia 2022-{_yr_fin} (diferencial < −10 pp). Frena aunque técnicamente no cae."),
        ("↓ EN DECLIVE", "FFCDD2", "B71C1C",
         f"Var. {_yr_ant}→{_yr_fin} negativa y peor de lo que su tendencia post-pandemia hacía esperar "
         f"(diferencial < −10 pp). La caída es una sorpresa negativa incluso descontando el efecto pandemia."),
        ("↓↓ CONTRACCION", "EF9A9A", "B71C1C",
         f"Var. {_yr_ant}→{_yr_fin} negativa pero dentro de lo esperable frente a su tendencia "
         f"post-pandemia (diferencial ≥ −10 pp)."),
        ("— SIN DATO", "EEEEEE", "757575",
         f"No hay dato de primer_curso en {_yr_ant} o {_yr_fin}, o faltan las variaciones "
         f"2022-{_yr_fin} necesarias para el AAGR post-pandemia."),
    ]
    ...
    # ── Fix 42: tabla equivalente para la señal post-pandemia (Fix 26) ────────
    fila += 1
    ws.merge_cells(f"A{fila}:N{fila}")
    _cell(
        ws, fila, 1,
        "SEÑAL DE TENDENCIA (POST-PANDEMIA) — Misma lógica, base de comparación distinta",
        bold=True, bg="1565C0", fg=BLANCO, size=10,
    )
    ws.row_dimensions[fila].height = 20
    fila += 1
    ...
    _cell(
        ws, fila, 1,
        f"Usa exactamente el mismo árbol de decisión de la tabla anterior, pero el diferencial se "
        f"calcula contra AAGR_post_pandemia (promedio de las variaciones 2022→2023 y 2023→{_yr_fin}) "
        f"en vez del AAGR_ROBUSTO de {AÑO_INICIO_HISTORICO}-{_yr_fin}. Esto evita que la caída de "
        f"2019→2020 por pandemia y el rebote 2020→2021 distorsionen la comparación para categorías "
        f"con base 2019 anómala. Las etiquetas (ACELERANDO, DESACELERANDO, etc.) significan lo mismo "
        f"que arriba; solo cambia contra qué historia se comparan.",
        bg="E3F2FD", fg="0D47A1", halign="left", wrap=True, size=9,
    )
    ws.row_dimensions[fila].height = 56
    fila += 1
    ...
    for señal, bg, fg, desc in SEÑALES_POST_PANDEMIA:
        _cell(ws, fila, 1, señal, bold=True, bg=bg, fg=fg)
        _cell(ws, fila, 2, desc, bg=bg, fg=fg, halign="left", wrap=True)
        ws.row_dimensions[fila].height = 28
        fila += 1
    ...
    _cell(
        ...
        f"'AAGR proyectado' = (Proyección {_año_proy_fin} / ajuste del modelo en "
        f"{AÑO_FIN_DATOS}) ^ (1/{HORIZONTE_PROYECCION_ANOS}) − 1 (Fix 44: se compara "
        f"contra lo que el propio modelo predice en {AÑO_FIN_DATOS}, no contra el dato "
        "real, para que el signo de la tasa sea siempre consistente con la trayectoria "
        "proyectada). "
        "'Tipo de proyección' usa umbrales propios calibrados por percentiles reales "
        "de esta tasa (Fix 45, no los del AAGR histórico): "
        "↑↑ EXPANSIÓN (>5.7%) · ↑ CRECIENTE (1.1% a 5.7%) · → ESTABLE (0% a 1.1%) · "
        "↓ DECRECIENTE (−5.4% a 0%) · ↓↓ CONTRACCIÓN (≤−5.4%). "
        "R² del ajuste indica la fiabilidad estadística del modelo (0-1).",
        bg="ECEFF1", fg="212121", size=9, halign="left", wrap=True,
    )
    ws.row_dimensions[fila].height = 88
...
def _escribir_hoja_total(
    ...
    NOMBRES_LEGIBLES["AAGR_PROYECTADO"] = f"AAGR proyectado ({AÑO_FIN_DATOS + 1}-{_AÑO_FIN_PROYECCION_FIX26})"
